...
import { getSongDeduplicationKey, areTracksEqual, detectLanguage } from '../services/musicNormalizationService';
...
export function useMusicApp() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('vd_past_queue_v1', JSON.stringify(pastQueue));
    } catch {}
  }, [pastQueue]);
      ...
      personalizationService.generatePersonalizedQueue(seedTrack, tracks, fullHistoryAndQueue, playbackContext)
        .then((newBatch) => {
          if (newBatch && newBatch.length > 0) {
            setOriginalQueue((prevOriginal) => {
              const currentKeys = new Set([
                getSongDeduplicationKey(currentTrack),
                ...pastQueue.map((t) => getSongDeduplicationKey(t)),
                ...prevOriginal.map((t) => getSongDeduplicationKey(t))
              ]);
              const freshTracks = newBatch.filter((t) => 
                !currentKeys.has(getSongDeduplicationKey(t)) && 
                !personalizationService.isDisliked(t.id)
              );
              if (freshTracks.length === 0) return prevOriginal;
              
              if (isShuffle) {
                setShuffledQueue((prevShuffled) => {
                  const shuffledKeys = new Set([
                    getSongDeduplicationKey(currentTrack),
                    ...pastQueue.map((t) => getSongDeduplicationKey(t)),
                    ...prevShuffled.map((t) => getSongDeduplicationKey(t))
                  ]);
                  const freshForShuffle = newBatch.filter((t) =>
                    !shuffledKeys.has(getSongDeduplicationKey(t)) &&
                    !personalizationService.isDisliked(t.id)
                  );
                  return [...prevShuffled, ...smartShuffle(freshForShuffle, currentTrack)];
                });
              }
              return [...prevOriginal, ...freshTracks];
            });
          }
        })
        .catch(() => {})
  ...
  const handleSelectTrack = async (
    ...
    // Stop previous audio immediately to prevent bleed
    ...
    // If selected from search or user chose track in search context, lock language to this song's language
    if (activeContext?.type === 'search' || (!keepQueue && activeContext?.title?.toLowerCase().includes('search'))) {
      const detectedLang = track.language || detectLanguage(track.title, track.artist, track.genre);
      personalizationService.setLockedLanguage(detectedLang);
    }
    ...
    if (!keepQueue) {
      ...
      if (isShuffle) {
        setShuffledQueue(overrideQueue);
      } else {
  ...
  const handleNextTrack = (isErrorSkip: boolean = false) => {
    // Instantly kill prior audio stream
    audioEngine.stopImmediate();
    setCurrentTimeSec(0);
  ...
  return {
    ...
    favoriteTrackIds, handleToggleFavorite,
    tracks,
    ...
    pastQueue, playbackContext,
    playbackHistory, handleClearHistory, handleRemoveFromHistory,
    personalizedHome, activeVibe, vibeToast, setVibeToast,
    repeatMode, handleToggleRepeat,
    isInfiniteAutoPlay, handleToggleInfiniteAutoPlay,
    radioTracks,
    isOfflineOnly, activeDisplayTracks,
    handleSelectTrack, handleNextTrack, handlePrevTrack, handlePlayQueue,
    handlePlayFromQueue, handleRemoveFromQueue, handleReorderQueue,
    handlePlayNext, handleAddToQueue, handleClearQueue, handleShuffleQueue, handlePrioritizeQueue,
    handlePlayMix, handlePlayVideoAudioOnly
  };
}
